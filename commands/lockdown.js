...
module.exports.run = async (bot, message, args) => {
  ...
  if(locks.has(message.channel.id) && !valid.includes(args[0].toLowerCase())) return message.channel.send(`This channel is already in lockdown!`)
  if(locks.has(message.channel.id) && valid.includes(args[0].toLowerCase())){
