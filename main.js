...
bot.on('message', async message => {
   ...
   switch(message.content.toLowerCase()){
    ...
    let now = Date.now(),
        ...
        let commands = await message.content.toLowerCase().replace(`<@${bot.user.id}>`, prefix.toLowerCase()).replace(`<@!${bot.user.id}>`, prefix.toLowerCase()).replace(' ', '').split(' ')
    if(!commands[0].startsWith(prefix.toLowerCase())) return;
    if(limit != null) {
    ...
    if(commands[0].startsWith(prefix.toLowerCase())){
        let cmd = commands[0].slice(prefix.length)
        let args = message.content.split(" ").slice(1);
        let commandfile = bot.commands.get(cmd.replace(args, ''));
        if (commandfile){
